...
=======
>>>>>>> 9a4c1efdb07033a6db13f753af12a8ec8224343b
raw$group="original"
results$group="corrected"

test <- rbind(raw,results)

ggplot(data=test[test$input_type!="15-30 + 7.5-15",],aes(y=modelled_SOC,x=input_type,fill=group))+
...
>>>>>>> 9a4c1efdb07033a6db13f753af12a8ec8224343b
```
## Rendu ici pour adaptation vers Trt-based process
...
write.csv(initial.stocks,paste0(main.dir,"data//prepared data//IPCCT2//initial_stocks_all.csv"),row.names = F)
```
...
initial.stocks<- read.csv(paste0(main.dir,"data//prepared data//IPCCT2//initial_stocks_all.csv"))
...
available.climate<-unique(climate_data_ipcct2$POLYID)

poly.index<-poly.index %>% filter(POLYID %in% available.climate)
...
site_data_driver <- site_data_driver %>% 
  filter(!is.nan(mean_c_input) & !is.na(mean_c_input) & !is.na(modelled_SOC) & !is.na(mean_temp) & !is.na(init_active_frac))
